...
const REQUIRED_FLOWS = [
	...
	'.maestro/flows/_shared/onboarded.yml',
	'.maestro/flows/smoke/launch.yml',
	'.maestro/flows/smoke/onboarding-to-deck.yml',
	'.maestro/flows/native/cold-relaunch.yml',
	'.maestro/flows/native/model-packs-absent.yml',
	'.maestro/flows/native/permissions-notifications.yml',
	'.maestro/flows/native/resolve-a-nudge.yml',
...
// variant-prefixed and optionally `!`-suffixed. no accessible name in recess looks like this
const TAILWIND_HEAD = [
...
describe('maestro flow files', () => {
	it('covers every flow the suite is built on', () => {
	...
	it('targets only the recess bundle id', () => {
	...
	it('avoids the commands that cannot fail CI honestly', () => {
		for (const { name, flow } of everyCommand) {
	...
	it('never keys a selector off a data-testid, a dom id, a css selector, or a tailwind class', () => {
		for (const { name, payload, flow } of everyCommand) {
			for (const { key, value } of selectorsForCommand(name, payload)) {
				...
				// WKWebView publishes no resource-id for DOM content, so an `id:` selector
				// silently never matches on iOS however stable the DOM id is
	...
	it('keeps config.yaml as the only .yaml in the workspace', () => {
		// maestro documents the workspace config as config.yaml only; a .yml is ignored, and
		// silently running with no workspace config is worse than failing here
	...
	it('holds nothing but yaml, so no binary lands in the workspace', () => {
		const nonYaml = walk(MAESTRO_DIR)
	...
	/**
	 * recess has no account and no server, so the sky flows this suite was ported from carry
	 * concepts that cannot exist here. Asserting their absence stops a future copy-paste from
	 * reintroducing a flow that can only ever fail.
	 */
	it('carries no sign-in, oauth or quest concepts', () => {
		for (const flow of flows) {
			const body = JSON.stringify(flow.commands).toLowerCase();
			for (const absent of ['oauth', 'sign up', 'username', 'password', 'quest']) {
				expect(
					body,
					`${flow.rel} mentions "${absent}", which recess has no surface for`
	...
	/**
	 * recess registers no CFBundleURLTypes, so `openLink` with a custom scheme would open a
	 * browser rather than routing into the app. Its navigation entry points are the three iOS
	 * home-screen shortcut items, and maestro has no command to invoke one - so deep-link
	 * coverage is genuinely unreachable here and must not be faked.
	 */
	it('never tries a custom-scheme deep link, which this app does not register', () => {
		for (const { name, payload, flow } of everyCommand) {
			if (name !== 'openLink') continue;
			const link = typeof payload === 'string' ? payload : (payload as { link?: string })?.link;
			expect(link, `${flow.rel}: openLink ${link} - recess registers no url scheme`).not.toMatch(
				/^com\.earthapp\.recess:/
			);
		}
	});
...
describe('maestro workspace config', () => {
	...
	it('declares the recess bundle id and the two runnable suites', () => {
...
describe('selector guards', () => {
	...
	it('leaves real accessible names alone', () => {
		for (const value of [
			'Today',
			'Playground',
			...
			'Grown from 0 Nudges',
			'Mark it Done Myself',
			'This Week is Still Blank',
			'Small Nudges. Real Life.',
			"A Few a Day. Then you're Done.",
			'On-Device Models',
			'Show a Friend',
			'Not Now',
			'Back'
	...
	it('flags testid selectors in any casing', () => {
		expect(looksLikeTestId('data-testid=today-ready')).toBe(true);
		expect(looksLikeTestId('[data-test="x"]')).toBe(true);
		expect(looksLikeTestId('Today')).toBe(false);
	});
});
